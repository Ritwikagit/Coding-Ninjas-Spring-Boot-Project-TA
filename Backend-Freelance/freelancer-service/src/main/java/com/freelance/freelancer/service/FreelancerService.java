...
public class FreelancerService {
    ...
    @Autowired
    private RestTemplate restTemplate; // make sure you have a RestTemplate bean
    @Transactional
...
//    }
    ...
    public List<FreelancerDTO.FreelancerMatch> getMatchingFreelancers(Set<String> requiredSkills) {
        ...
        Set<String> lower = requiredSkills.stream()
        ...
        return skillRepo.findBySkillNameIn(lower).stream()
                ...
                .map(e -> {
                    ...
                    // Call auth microservice to get freelancer name
                    String url = "http://localhost:8081/auth/profile/" + fid;
                    String name;
                    try {
                        name = restTemplate.getForObject(url, String.class);
                    } catch (Exception ex) {
                        name = "Unknown"; // fallback if service fails
                    }
